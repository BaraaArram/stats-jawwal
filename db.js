...
async function createDatabase(dbPath) {
  ...
  if (JSONFile && Low) {
    ...
    return {
      ...
      async deleteItem(collection, key, value) {
        ...
        const [removed] = low.data[collection].splice(existingIndex, 1);
        await low.write();
        return removed;
      },
  ...
  if (legacyLow && FileSyncAdapter) {
    ...
    return {
      ...
      async listCollection(collection) {
        return dbLegacy.get(collection).value() || [];
      },
      async deleteItem(collection, key, value) {
        const found = dbLegacy.get(collection).find({ [key]: value }).value();
        if (!found) {
        ...
        dbLegacy.get(collection).remove({ [key]: value }).write();
        return found;
      },
